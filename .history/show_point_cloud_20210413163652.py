...
def line_set(points):
    ...
    a = np.array([0.46790581,  0.04191191, -0.88278398])
    b = np.array([-0.88275161,  0.07029837, -0.4645511])
    c = np.cross(eigenvectors[:, 0], eigenvectors[:, 1])
    res = [a.dot(b.T), b.dot(c.T), c.dot(a.T)]
    print(res)

    # 在原点云中画图
    point = [medium_point, a, b, c]  # 提取第一v和第二主成分 也就是特征值最大的对应的两个特征向量 第一个点为原点
