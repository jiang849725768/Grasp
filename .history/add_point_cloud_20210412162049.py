...
def line_set(points):
    ...
    # points = points.T
    points = DataFrame(points[:, 0:3])  # 选取每一列 的 第0个元素到第3个元素
    ...
    eigenvectors[:, 0] = [0.46790581,  0.04191191, -0.88278398]
    eigenvectors[:, 1] = [-0.88275161,  0.07029837, -0.4645511]
    c = np.cross(eigenvectors[:, 0], eigenvectors[:, 1])
    res = eigenvectors[:, 0].dot(eigenvectors[:, 1].T)
    print(res)

    # 在原点云中画图
    point = [medium_point, eigenvectors[:, 0],
             eigenvectors[:, 1], c]  # 提取第一v和第二主成分 也就是特征值最大的对应的两个特征向量 第一个点为原点
    lines = [[0, 1], [0, 2], [0, 3]]  # 有点构成线，[0, 1]代表点集中序号为0和1的点组成线，即原点和两个成分向量划线
