...
def rm2rv(R):
    ...
    print(theta)
    K = (1.0 / (2.0 * np.sin(theta))) * np.asarray(
...
def main():
    ...
    rotation_matrix = feature_vector[[1, 2, 0], :].T
    print(rotation_matrix)
    print(f"两两相乘：{rotation_matrix[0].dot(rotation_matrix[1].T)}")
    print(f"两两相乘：{rotation_matrix[1].dot(rotation_matrix[2].T)}")
    print(f"两两相乘：{rotation_matrix[0].dot(rotation_matrix[2].T)}")
    ...
    print(medium_point, rotation_vector.T)
