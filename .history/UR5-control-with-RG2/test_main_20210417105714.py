...
def move_to_tcp(target_tcp):
    ...
    tcp_command = "movel(p[%f,%f,%f,%f,%f,%f],a=%f,v=%f,t=0,r=0)\n" % (
        ...
        target_tcp[4], target_tcp[5], tool_acc, tool_vel)
    tcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    tcp_socket.connect((HOST, PORT))
    tcp_socket.send(
        str.encode(tcp_command))  # 利用字符串的encode方法编码成bytes，默认为utf-8类型
    tcp_socket.close()
    ...
    while not (all([
            np.abs(actual_pos[j] - target_tcp[j]) < tool_pos_tolerance[j]
            for j in range(3)
    ]) and all([
            np.abs(rpy[j] - target_rpy[j]) < tool_pos_tolerance[j + 3]
            for j in range(3)
    ])):
        actual_pos = get_current_tcp()
        rpy = util.rv2rpy(actual_pos[3], actual_pos[4], actual_pos[5])
...
def increase_move(delta_x, delta_y, delta_z, delta_theta):
    ...
    target_tcp = np.asarray([
        tcp[0] + delta_x, tcp[1] + delta_y, tcp[2] + delta_z, target_rv[0],
        target_rv[1], target_rv[2]
    ])
...
def go_home():
    ...
    move_to_tcp([
        0.20376213, -0.45702899, 0.34746897, 1.04741165, 1.69086425,
        -1.02765625
    ])
...
if __name__ == '__main__':
    go_home()
    # [ 0.03778355 -0.33591624  0.80009833] -0.5557693   0.52376203  0.19781328
    # print(get_current_tcp())
    current_tcp = get_current_tcp()
    move_tcp = np.hstack(([0.05748436, -0.8097384,   0.28349802], current_tcp[3:]))
    move_to_tcp(move_tcp)

    # move_to_tcp(
    #     [0.01857354, -0.78721165,  0.28067099, 0.45975248, 0.5331555,  0.17068289])
